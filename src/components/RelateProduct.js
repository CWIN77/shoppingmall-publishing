  ...
  const productData = [
    {
      ...
      percent:56,
      beforePrice:15900,
      img:'https://always-bucket-dev.s3.us-west-1.amazonaws.com/3ab6f2de-d35f-489d-bd34-5d30ec675239_original.jpg',
      score:[2,2,2,2,1],
      countReview:11872
    },
    {
            ...
            return (
              <Product key={key}>
                ...
                <div style={{display:'flex',margin:'4px 0px'}}>
                  {
                    data.tag.map((tag,key)=>{
                      if(key < 2){
                        return <Tag key={key} color={tag.color}>{tag.name}</Tag>
                      }
                    })
                  }
                </div>
                ...
                <div style={{display:'flex'}}>
                  <h2 style={{color:'rgb(102, 102, 102)'}}>{data.percent}%</h2>
                  <BeforePrice>{data.beforePrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}원</BeforePrice>
                </div>
                <Price>{data.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}원</Price>
                <Grade>
                  {
                    data.score.map((score,key)=>{
                      if(score === 2) return <StarIcon key={key} src={fullStar}/>
                      else if(score === 1) return <StarIcon key={key} src={halfStar}/>
                      else if(score === 0) return <StarIcon key={key} src={emptyStar}/>
                    })
                  }
                  ({data.countReview})
                </Grade>
